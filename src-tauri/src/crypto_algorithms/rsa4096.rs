/*
  ...
  说明：
  - 文本：优先尝试 RSA-OAEP 直接加密；超出长度限制时自动切换为混合加密（RSA 包裹会话密钥 + AEAD 加密正文）。
  - 文件：使用混合加密（RSA 包裹会话密钥 + 流式 AEAD 加密文件数据）。
*/
...
fn normalize_parts(parts: Vec<keystore::KeyPart>) -> Result<Vec<keystore::KeyPart>, String> {
    ...
    let private_norm = if let Some(p) = private_in {
        ...
        if private_key.n().bits() as usize != 4096 {
            return Err(format!(
                "RSA 密钥位数不匹配：期望 4096，实际 {}",
                private_key.n().bits()
            ));
        }
    ...
    let public_norm = if let Some(p) = public_in {
        ...
        if pub_key.n().bits() as usize != 4096 {
            return Err(format!(
                "RSA 密钥位数不匹配：期望 4096，实际 {}",
                pub_key.n().bits()
            ));
