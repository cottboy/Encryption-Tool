...
mod tests {
    /*
      单元测试（密钥库加解密封装）：
      - 验证“加密→解密还原数据”与“错误密码失败”两条关键路径。
      - 注意：这里测试的是 KeyStore 容器本身，不涉及具体密钥算法。
    */

    use super::*;

    #[test]
    fn keystore_encrypt_then_decrypt_roundtrip() {
        let plain = KeyStorePlain {
            version: KEYSTORE_VERSION,
            key_entries: vec![KeyEntry {
                ...
                material: KeyMaterial::Symmetric {
                    key_b64: B64.encode([7u8; 32]),
                },
            }],
        };

        let (file, kdf, _key) = encrypt_with_new_password(&plain, "pw123").expect("加密应成功");
        ...
        match file {
            KeyStoreFile::Encrypted {
                ...
                let (decoded, _derived_key) =
                    decrypt_keystore_return_key(&kdf, &aead, &ciphertext_b64, "pw123").expect("解密应成功");
                assert_eq!(decoded.version, plain.version);
                assert_eq!(decoded.key_entries.len(), 1);
                assert_eq!(decoded.key_entries[0].key_type, "AES-256");
            }
            _ => panic!("预期为加密格式"),
        }
    }
    ...
    #[test]
    fn keystore_wrong_password_should_fail() {
        let plain = KeyStorePlain {
            version: KEYSTORE_VERSION,
            key_entries: Vec::new(),
        };

        let (file, kdf, _key) = encrypt_with_new_password(&plain, "right").expect("加密应成功");
        ...
        match file {
            KeyStoreFile::Encrypted {
                ...
                let err = decrypt_keystore_return_key(&kdf, &aead, &ciphertext_b64, "wrong")
                    .err()
                    .expect("错误密码应失败");
                assert!(matches!(err, KeyStoreError::PasswordOrDataInvalid));
            }
            _ => panic!("预期为加密格式"),
        }
    }
